...
public:
  ...
  Matrix(std::size_t N, std::size_t M)
      : N(N),
        M(M),
        data(std::vector<std::vector<double>>(N, std::vector<double>(M))) {}
...
bool operator==(const Matrix& left, const Matrix& right) {
  if (left.getN() != right.getN() || left.getM() != right.getN()) {
    return false;
  }
  for (std::size_t i = 0; i < left.getN(); i++) {
    for (std::size_t j = 0; j < right.getM(); j++) {
